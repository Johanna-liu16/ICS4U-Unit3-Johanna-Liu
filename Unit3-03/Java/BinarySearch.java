...
final class BinarySearch {
  ...
  */
  static int binarySearch(final int[] userArray, final int userNumber,
    ...
    int half = (highIndex + lowIndex) / 2;
    int value = 0;

    if (userNumber == userArray[half]) {
      value = half;
      return value;
    } else if (userNumber > userArray[half]) {
      value = binarySearch(userArray, userNumber, half, highIndex);
    } else {
      value = binarySearch(userArray, userNumber, lowIndex, half);
    }
    return value;
  }
